...
RSpec.describe ManagerRequestApiSchema, type: :graphql do
  ...
  def execute(query, variables = {})
    context = { current_user: user, current_account: account, current_ability: Ability.new(user) }
  ...
  describe 'approveRequest' do
    let(:request) { create(:request, account: account, user: user, category: category, status: :pending_approval) }
  ...
  describe 'destroyComment' do
    let(:comment_and_request) do
      ...
      { request: req, comment: create(:comment, account: account, request: req, user: user, body: 'Comentário') }
    end
    ...
    it 'destroys a comment' do
      result = execute(query, { 'id' => comment_and_request[:comment].id.to_s })

      expect(result['errors']).to be_blank
      expect(result.dig('data', 'destroyComment', 'errors')).to eq([])
      expect(result.dig('data', 'destroyComment', 'comment', 'id')).to eq(comment_and_request[:comment].id.to_s)
    end
  end
end
